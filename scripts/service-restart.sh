...
if [[ "$1" == "master" ]]; then
	...
	curl -X POST -H 'Content-type: application/json' --data '{"text":"Senti Tag Manager MASTER updated and restarted!"}' $2
	echo
	exit 0
fi
...
if [[ "$1" == "dev" ]]; then
	...
	# Senti Slack Workspace
	curl -X POST -H 'Content-type: application/json' --data '{"text":"Senti Tag Manager DEV updated and restarted!"}' $2
	echo
	exit 0
fi
