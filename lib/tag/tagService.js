...
class tagService {
	...
	async editTag(tag = null) {
		...
		let updateSQL = `UPDATE tag
						 ...
						 WHERE uuid=?;`
		let updateQuery = await this.db.query(updateSQL, [tag.name, tag.description, tag.color, tag.uuid])
		if (updateQuery[0].affectedRows === 1) {
			let ftag = await this.getTagByUuid(tag.uuid)
			return ftag
		}
		else {
			return false
	...
	async deleteTag(uuid = null) {
		...
		let deleteSQL = `UPDATE tag SET deleted=1 WHERE uuid=?;`
	...
	async createResourceTag(rtag = null) {
		...
		console.log('rtagExists', rtagExists)
		if (rtagExists.uuid) {
			console.log('Resource tag exists', rtagExists)
	...
	async getTagById(id = null) {
		...
		let selectSQL = `SELECT id, uuid, name, description, color, appId, orgId FROM tag t where id=? and deleted=0`
	...
	async getTagByUuid(uuid = null) {
		...
		let selectSQL = 'SELECT id, uuid, name, description, color FROM tag where uuid=? and deleted=0'
	...
	async getResourceTagById(id = null) {
		...
		let selectSQL = 'SELECT id, uuid, tagUUID, resourceUUID, resourceType FROM tagResource where id=? and deleted=0'
		let selectQuery = await this.db.query(selectSQL, [id])
	...
	async getResourceTagByUuid(uuid) {
		...
		let selectSQL = 'SELECT id, uuid, tagUUID, resourceUUID, resourceType FROM tagResource where uuid=? and deleted=0'
		let selectQuery = await this.db.query(selectSQL, [uuid])
		return new ResourceTag(selectQuery[0][0])
	}
	...
	async getRTagsByTagUUID(tagUUID = null) {
		...
		let selectSQL = `SELECT * from tagResource where tagUUID = ? and deleted=0`
	...
	async getRTagByTagUUIDandResourceUUID(tagUUID = null, resourceUUID = null) {
		...
		let selectSQL = `SELECT * from tagResource where tagUUID = ? and resourceUUID = ? and deleted=0`
	...
	async getTagsByResourceUuid(uuid = null) {
		...
		let selectSQL = `SELECT * from tag t
						 ...
						 WHERE tr.resourceUUID = ? and tr.deleted = 0 and t.deleted = 0`
		let selectQuery = await this.db.query(selectSQL, [uuid])
	...
	async getTagsByAppIdOrgId(appId, orgId) {
		let sql = `SELECT * from tag where appId=? and orgId=? and deleted=0`
